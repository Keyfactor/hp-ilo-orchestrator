    ...
    public class Reenrollment : OrchestratorJob<Reenrollment>, IReenrollmentJobExtension
    {
        ...
        public JobResult ProcessJob(ReenrollmentJobConfiguration config, SubmitReenrollmentCSR submitReenrollment)
        {
            ...
            JobResult result = new()
            {
                ...
                JobHistoryId = JobConfig.JobHistoryId.Value,
                FailureMessage = "Reenrollment failed."
            };
            X509Certificate2 returnCert;
            string CSRstring = "";

            HPiLOClient APIclient = APIService.GetRequiredService<HPiLOClient>();
            CSRType CertType = ReenrollmentCertType(ExtractCN(JobConfig.JobProperties["subjectText"].ToString()));
            ...
            try
            {
                ...
                if (CertType == CSRType.HTTPSCert)
                {
                    ...
                    address = "/redfish/v1/Managers/1/SecurityService/HttpsCert/Actions/HpeHttpsCert.GenerateCSR";
                }
                ...
                else if (CertType == CSRType.iLOLDevID)
                {
                    ...
                    APIclient.DeleteCertificate("1/ILoLDevID");
                    // generating the actual csr
                    address = "/redfish/v1/CertificateService/Actions/CertificateService.GenerateCSR";
                }
                else
                {
                ...
                CSRstring = APIclient.GenerateCSR(address, JobConfig.JobProperties["subjectText"].ToString(), CertType);
            }
            catch (Exception ex)
            {
            ...
            // step 2) submit the CSR to the Command Platform via Orchestrator hook
            try
            {
            ...
            try
            {
                if (CertType == CSRType.HTTPSCert)
                {
                    bool certHasPrivateKey = returnCert.HasPrivateKey;
                    // If private key was passed down with the certificate from Keyfactor
                    if (certHasPrivateKey)
                    {
                        logger.LogDebug("Importing HTTPS certificate with private key");
                        if (APIclient.ImportCertificate(ExportCertificateWithDetails(returnCert),
                    ...
                    // If NO private key was passed down with the certificate from Keyfactor
                    else
                    {
                        logger.LogDebug("Importing HTTPS Cert without private key");
                        if (APIclient.ImportCertificate(ToPem(returnCert.RawData),
                                "/redfish/v1/Managers/1/SecurityService/HttpsCert/Actions/HpeHttpsCert.ImportCertificate",
                                CertType))
                        {
                            result.Result = OrchestratorJobStatusJobResult.Success;
                ...
                {
                    logger.LogDebug("Importing iLOLDevID certificate");
                    if (APIclient.ImportCertificate(ToPem(returnCert.RawData),
                            "/redfish/v1/Managers/1/SecurityService/iLOLDevID/Certificates/", CertType))
                    {
                        result.Result = OrchestratorJobStatusJobResult.Success;
                        result.FailureMessage =
                            $"Successfully performed reenrollment on certificate with subject `{returnCert.SubjectName.Name}`";
        ...
        // Performs all ncessary operations for HTTPSCert reenrollment
        ...
        public static string ExtractCN(string distinguishedName)
        {
            ...
            int index = distinguishedName.IndexOf(cnPrefix, StringComparison.OrdinalIgnoreCase);
            if (index == -1)
            {
                return string.Empty;
            }

            int startIndex = index + cnPrefix.Length;
            int endIndex = distinguishedName.IndexOf(',', startIndex);
            if (endIndex == -1)
            {
                endIndex = distinguishedName.Length;
            }

            string cn = distinguishedName.Substring(startIndex, endIndex - startIndex);
            return cn.Trim();
        }

        // Function to determine what we are reenrolling.
        public CSRType ReenrollmentCertType(string input)
        {
            if (string.IsNullOrEmpty(input))
            {
                throw new Exception("Incorrect subject string.");
            }

            CSRType certType = CSRType.HTTPSCert;
            if (input.StartsWith("ILO", StringComparison.OrdinalIgnoreCase))
            {
                certType = CSRType.HTTPSCert;
            }
            else
            {
                certType = CSRType.iLOLDevID;
            }

            return certType;
        }

        public void ReenrollmentInitialize(ReenrollmentJobConfiguration rnrConfig)
        {
            ...
            JobConfig.ServerUsername =
                PamResolver.ResolvePAMField(PamSecretResolver, logger, "Server Username", rnrConfig.ServerUsername);
            JobConfig.ServerPassword =
                PamResolver.ResolvePAMField(PamSecretResolver, logger, "Server Password", rnrConfig.ServerPassword);
