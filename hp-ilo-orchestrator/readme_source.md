...
##Testing Framework
The testing framework requires an installation of OpenSSH for Windows on the target machine. Additionally, the contents of the ```scripts``` folder must be added
to an Azure Labs Keyfactor install folder, as well as a valid azurelabs.cmslicense file. 
